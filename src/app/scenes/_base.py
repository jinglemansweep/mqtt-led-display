...
class BaseScene:
    ...
    async def configure_hass_entity(self, name, device_class='switch', options=None):
        ...
        auto_config = dict(
            name=full_name, unique_id=full_name, device_class=device_class, schema='json',            
            command_topic=f'{topic}/set', state_topic=f'{topic}/state',
        )
        config = auto_config.copy()
        config.update(options)
        print(f'HASS Config: {full_name} [{device_class}] > {config}')
        await self.manager.client.publish(f'{topic}/config', json.dumps(config), retain=True, qos=1)